...
def load_subjects_from_folder():
    """'study_materials' 폴더의 하위 폴더 이름을 과목으로 로드합니다."""
...
def draw_random_subject():
    ...
    if not drawable_subjects:
        if all_available_subjects:
            messagebox.showwarning("오 오류", "랜덤으로 뽑을 수 있는 과목이 없습니다.\n"
...
def record_from_main_ui():
    ...
    global last_chosen_subject # 이 함수에서 last_chosen_subject 값을 참조하므로 global 선언이 필요합니다.
